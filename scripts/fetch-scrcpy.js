 ...
 * годится для сборки "по архитектуре хоста", но electron-builder собирает
 * ОБА mac-архива (x64 и arm64, см. package.json build.mac.target[0].arch)
 * из одного и того же vendor/mac за один запуск -- то есть даже после
 * перехода с одного universal-приложения на два отдельных по архитектуре
 * (вдвое меньше скачивать) сам vendor/mac всё равно ровно один на оба
 * выходных файла, а не два арх-специфичных. Раз он один общий -- проще и
 * надёжнее держать universal-бинарник внутри него, чем городить отдельные
 * vendor/mac-x64 и vendor/mac-arm64 ради экономии нескольких МБ scrcpy на
 * фоне десятков-сотен МБ самого Electron. lipo -create сшивает два однооархитектурных
